...
import { FiSearch, FiEye, FiMessageSquare, FiPlus, FiClock, FiCheckCircle } from 'react-icons/fi';

const InvestorSearchStartups = () => {
    ...
    const loadAllProposals = async () => {
        setLoading(true);
        try {
            const data = await investorService.getProposals();
    ...
    const handleRequestAccess = async (proposalId) => {
        ...
        try {
            await investorService.requestAccess(proposalId, message);
    ...
    const handleAddInvestment = async (proposalId) => {
        const amount = prompt('Enter the investment amount (₹):');
        if (!amount) return;
        
        try {
            await investorService.addToPortfolio(proposalId, parseFloat(amount));
            setSuccess('Investment added to portfolio!');
            setTimeout(() => setSuccess(''), 3000);
            await loadAllProposals();
        } catch (err) {
    ...
    const handleViewProposal = (proposalId) => {
        navigate(`/investor/proposal/${proposalId}`);
    };
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            case 'INVESTED':
                return <span className="inline-flex items-center gap-1 text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full text-xs font-medium"><FiClock /> Invested</span>;
            default:
                        ...
                        <div key={proposal.startupId} className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition p-6">
                            <div className="flex flex-wrap justify-between items-start gap-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <button
                                        onClick={() => handleAddInvestment(proposal.startupId)}
                                        className="flex items-center gap-1 px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-600 text-sm font-medium rounded-lg transition"
                                    >
                                        <FiPlus /> Invest
                                    </button>
                                </div>
...
export default InvestorSearchStartups;
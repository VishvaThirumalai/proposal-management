...
public class MentorController {
    ...
    public ResponseEntity<?> getProposals() {
        try {
            log.info("📋 Mentor fetching all proposals");
            ...
            for (Startup p : proposals) {
                ...
                User mentor = userRepository.findByEmail(email).orElse(null);
                if (mentor != null) {
                    Optional<Request> existing = requestRepository.findByStartupStartupIdAndRecipientUserId(
                            p.getStartupId(), mentor.getUserId());
    ...
    public ResponseEntity<?> searchProposals(@RequestParam("q") String query) {
        try {
            log.info("🔍 Mentor searching proposals: {}", query);
    ...
    public ResponseEntity<?> getRecommendations() {
        try {
            ...
            log.info("📊 Getting recommendations for mentor: {}", mentor.getName());
    ...
    public ResponseEntity<?> getRequests() {
        try {
            ...
            log.info("📬 Getting requests for mentor: {}", mentor.getName());

            List<Request> requests = requestRepository.findByRecipientUserId(mentor.getUserId());
    ...
    public ResponseEntity<?> respondToRequest(
        ...
        try {
            ...
            log.info("📝 Mentor {} responding to request: {}", mentor.getName(), requestId);

            Request request = requestRepository.findById(requestId)
                    .orElseThrow(() -> new RuntimeException("Request not found"));

            if (!request.getRecipient().getUserId().equals(mentor.getUserId())) {
            ...
            if (requestDTO.isAccept()) {
                ...
                log.info("✅ Mentor {} accepted request: {}", mentor.getName(), requestId);
                ...
                log.info("❌ Mentor {} rejected request: {}", mentor.getName(), requestId);
    ...
    public ResponseEntity<?> getAssignedProposals() {
        try {
            ...
            log.info("📋 Getting assigned proposals for mentor: {}", mentor.getName());

            List<Request> acceptedRequests = requestRepository.findByRecipientUserIdAndStatus(
                    mentor.getUserId(), RequestStatus.ACCEPTED);
    ...
    // ================================================================
    // 7. VIEW PROPOSAL WITH PRE (FIXED)
    // ================================================================
...
public ResponseEntity<?> viewProposal(@RequestBody Map<String, String> request) {
    try {
        ...
        String mentorPrivateKey = request.get("privateKey");

        if (mentorPrivateKey == null || mentorPrivateKey.isEmpty()) {
        ...
        log.info("🔐 Mentor {} requesting to view proposal: {}", mentor.getName(), startupId);
        ...
        Optional<ReEncryptionKey> reKeyOpt = reEncryptionKeyRepository
                .findActiveByStartupAndUser(startupId, mentor.getUserId());
        ...
        // ✅ Get the re-encrypted AES key (RK)
        String reEncryptedAesKey = reKey.getEncryptedRk();
        log.debug("🔑 RK length: {}", reEncryptedAesKey.length());

        // ✅ Decrypt the AES key using mentor's private key
        // This returns a String that is actually the raw AES key bytes as a string
        String aesKeyString = ipfsService.decryptAESKeyWithPrivateKey(
            reEncryptedAesKey,
            mentorPrivateKey
        );
        log.debug("🔑 Decrypted AES key string length: {}", aesKeyString.length());

        // ✅ Convert to bytes directly (DO NOT Base64 decode!)
        byte[] aesKeyBytes = aesKeyString.getBytes();
        log.debug("🔑 AES key bytes length: {}", aesKeyBytes.length);

        // ✅ Ensure AES key is 32 bytes (256-bit)
        if (aesKeyBytes.length != 32) {
            log.warn("⚠️ AES key length is {} bytes, expected 32", aesKeyBytes.length);
            // If it's a Base64 encoded string, decode it
            try {
                aesKeyBytes = Base64.getDecoder().decode(aesKeyString);
                log.debug("🔑 Decoded Base64 AES key, length: {}", aesKeyBytes.length);
            } catch (IllegalArgumentException e) {
                log.warn("⚠️ Not Base64 encoded, using raw bytes");
                // If not Base64, use the raw bytes
                aesKeyBytes = aesKeyString.getBytes();
            }
        }

        SecretKey aesKey = new SecretKeySpec(aesKeyBytes, 0, Math.min(aesKeyBytes.length, 32), "AES");
    ...
    public ResponseEntity<?> checkAccessStatus(@PathVariable Long startupId) {
        try {
            ...
            log.info("🔍 Checking access status for mentor {} on startup {}", mentor.getName(), startupId);
    ...
    public ResponseEntity<?> getAccessLogs() {
        try {
            ...
            log.info("📋 Getting access logs for mentor: {}", mentor.getName());

            List<AccessLog> logs = accessLogRepository.findByUserUserId(mentor.getUserId());
